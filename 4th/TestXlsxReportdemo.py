...
from initmysqldata import init_data
...
def get_format_center(wd, num=1): # 设置居中
    return wd.add_format({'align': 'center', 'valign': 'vcenter', 'border': num})


def set_border_(wd, num=1):  #设置边框
...
worksheet = workbook.add_worksheet("Test summary")
worksheet2 = workbook.add_worksheet("Test cases details")
...
def pie(workbook, worksheet):
    ...
    chart1.add_series({
        'name':       '接口测试统计',
        'categories': '=Test summary!$D$4:$D$5',  # 此处的第一个字段必须为sheet name 老师的图没有生成就是因为名字不匹配
        'values':    '=Test summary!$E$4:$E$5',  # 名字匹配以后饼图就生成了，！前面为sheet name， 最好不要加空格。
    })
...
workbook.close()  # need add close, 否则看不到test report的生成
